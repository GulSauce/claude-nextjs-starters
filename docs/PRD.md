...
4. [Claude Code] /validate 커맨드 실행
   ...
   - Phase 1: Agent A/B 병렬 독립 평가 (6개 루브릭 기준)
   - Phase 2: 점수 교차 검토 (서로의 평가 결과를 검토)
   - Phase 2.5: 개선 프롬프트 교차 검토 (서로의 개선 프롬프트를 검토, F016)
   - Phase 3: 합의 조정자가 최종 점수/피드백/개선 프롬프트 결정
...
### 3. Phase 5 기능 (듀얼 에이전트 토론 평가)

| ID       | 기능명                  | 설명                                                                                                                                                                                                     | 관련 페이지      |
| -------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| **F013** | 듀얼 에이전트 토론 평가 | 프롬프트 엔지니어 + 교육 평가 전문가 2개 에이전트가 독립 평가 → 교차 검토 → 합의 도출하는 토론 프로세스                                                                                                  | Claude Code      |
| **F014** | 토론 로그 시각화        | 에이전트 간 대화를 채팅/메신저 스타일로 시각화 (Phase별 Accordion, 역할별 색상 말풍선)                                                                                                                   | 결과 상세 페이지 |
| **F015** | 합의 기반 점수 산정     | 합의 결과 + Agent A/B 점수 비교 바 차트 + 루브릭 항목별 3자 비교 테이블 + consensusSummary 표시                                                                                                          | 결과 상세 페이지 |
| **F016** | 개선 프롬프트 교차 검토 | Phase 2와 Phase 3 사이에 각 에이전트가 상대의 개선 프롬프트를 검토하는 Phase 2.5 단계 추가. 합의 조정자가 양쪽 피드백(강점/약점/개선안/필수 포함 요소)을 근거로 더 높은 품질의 최종 개선 프롬프트를 도출 | Claude Code      |
...
| **주요 기능**   | - **대기 상태**: data/results/{id}.json 없을 때 안내 메시지 + /validate 실행 안내 표시 (F012)<br>- **완료 상태**: 총점(0~100) + 등급 뱃지(A~F, 색상 구분) + 평가 모드 뱃지(Debate) 표시 (F005)<br>- **single 모드** (3탭): 항목별 점수 / 개선된 프롬프트 / 원본 프롬프트<br>- **debate 모드** (5탭): 합의 결과(F015) / 에이전트별 평가 / 토론 로그(F014) / 개선된 프롬프트 / 원본 프롬프트 |
...
### ValidationResult 듀얼 에이전트 확장 필드 (Phase 5, optional)

| 필드             | 설명                           | 타입/관계            |
| ---------------- | ------------------------------ | -------------------- |
| evaluationMode   | 평가 모드                      | "single" \| "debate" |
| agentEvaluations | 에이전트별 개별 평가 결과 배열 | AgentEvaluation[]    |
| debateRounds     | 토론 라운드 (Phase별)          | DebateRound[]        |
| debateLog        | 전체 토론 메시지 시간순 배열   | DebateMessage[]      |
| consensusSummary | 합의 요약 텍스트               | string               |

### AgentEvaluation (에이전트별 개별 평가 결과)

| 필드            | 설명             | 타입/관계                                  |
| --------------- | ---------------- | ------------------------------------------ |
| agentRole       | 에이전트 역할    | "prompt-engineer" \| "education-evaluator" |
| rubricScores    | 항목별 점수 배열 | RubricScore[]                              |
| totalScore      | 총점             | number                                     |
| grade           | 등급             | Grade                                      |
| overallFeedback | 종합 피드백      | string                                     |
| improvedPrompt  | 개선된 프롬프트  | string                                     |

### DebateMessage (토론 메시지)

| 필드      | 설명          | 타입/관계                                                                          |
| --------- | ------------- | ---------------------------------------------------------------------------------- |
| agent     | 에이전트 이름 | string                                                                             |
| role      | 에이전트 역할 | "prompt-engineer" \| "education-evaluator" \| "consensus-moderator"                |
| phase     | 토론 단계     | "independent-evaluation" \| "cross-review" \| "prompt-cross-review" \| "consensus" |
| content   | 메시지 내용   | string                                                                             |
| timestamp | 타임스탬프    | ISO 8601 string                                                                    |

### CrossReviewComment (교차 검토 코멘트)

| 필드           | 설명           | 타입/관계   |
| -------------- | -------------- | ----------- |
| reviewerRole   | 검토자 역할    | AgentRole   |
| targetRole     | 검토 대상 역할 | AgentRole   |
| criterionId    | 루브릭 항목 ID | CriterionId |
| originalScore  | 원래 점수      | number      |
| suggestedScore | 제안 점수      | number      |
| comment        | 조정 근거      | string      |

### DebateRound (토론 라운드)

| 필드        | 설명        | 타입/관계       |
| ----------- | ----------- | --------------- |
| phase       | 토론 단계   | DebatePhase     |
| messages    | 메시지 배열 | DebateMessage[] |
| startedAt   | 시작 시각   | ISO 8601 string |
| completedAt | 완료 시각   | ISO 8601 string |

### PromptReviewComment (개선 프롬프트 교차 검토 코멘트, Phase 6)

| 필드                | 설명                   | 타입/관계 |
| ------------------- | ---------------------- | --------- |
| reviewerRole        | 검토자 역할            | AgentRole |
| targetRole          | 검토 대상 역할         | AgentRole |
| strengths           | 강점 (1~5개)           | string[]  |
| weaknesses          | 약점 (1~5개)           | string[]  |
| suggestions         | 개선안 (1~5개)         | string[]  |
| mustIncludeElements | 필수 포함 요소 (1~5개) | string[]  |
